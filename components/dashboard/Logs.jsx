	...
	componentWillReceiveProps(nextProps) {
		if (this.props.device_list != nextProps.device_list) {
			...
			if (nextProps.device_list.length > 0) {
				...
				this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/devices-logs/'+devices+'/?format=json', function(result) {
					this.setState({
						c_logs: result,
					})
				}.bind(this))
				this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/devices-llogs/'+devices+'/?format=json', function(result) {
					this.setState({
						uc_logs: result,
					})
				}.bind(this))
				this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/devices-ewlogs/'+devices+'/?format=json', function(result) {
					this.setState({
						ew_logs: result,
					})
				}.bind(this))
