...
export default class Dash extends React.PureComponent {
	...
	getDeviceList() {
		this.serverRequest = $.get('http://192.168.0.11:8000/api/devices/?format=json', function(result) {
			this.setState({
				device_list: result,
			});
		}.bind(this))
	...
	handleData(data) {
		...
		this.serverRequest = $.get('http://192.168.0.11:8000/api/devices-details/'+devices+'/?format=json', function(data) {
			this.setState({
					...
					<div className="card bg-light">
						...
						{this.props.current_user.is_superuser &&
							<Modal open={this.state.open} onClose={this.closeModal.bind(this)} little>
								<button className="btn btn-block b-t-font m-bg-color">Flush Database</button>
							</Modal>
						}
						{!this.props.current_user.is_superuser &&
							<Modal open={this.state.open} onClose={this.closeModal.bind(this)} little>
