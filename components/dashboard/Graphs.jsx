	...
	componentWillReceiveProps(nextProps) {
		...
		if (this.props.device_list != nextProps.device_list) {
			...
			if (nextProps.device_list.length > 0) {
				...
				this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/devices-lchart/'+devices+'/?format=json', function(result) {
				...
				nextProps.device_list.map(function(device) {
					...
					if (index < 0) {
						this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/device-charta/'+device.id+'/?format=json', function(data) {
						...
						this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/device-chartb/'+device.id+'/?format=json', function(data) {
							data.forEach(function(d) {
								if (parseDate(d.day)) {
									d.day = parseDate(d.day)
								} else if (parseRealDate(d.day.substring(0, d.day.length-4))) {
						...
						this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/device-chartc/'+device.id+'/?format=json', function(data) {
							data.forEach(function(d) {
								if (parseDate(d.day)) {
									d.day = parseDate(d.day)
								} else if (parseRealDate(d.day.substring(0, d.day.length-4))) {
						...
						this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/device-chartd/'+device.id+'/?format=json', function(data) {
							data.forEach(function(d) {
								if (parseDate(d.day)) {
									d.day = parseDate(d.day)
								} else if (parseRealDate(d.day.substring(0, d.day.length-4))) {
						...
						this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/device-charte/'+device.id+'/?format=json', function(data) {
							data.forEach(function(d) {
								if (parseDate(d.day)) {
									d.day = parseDate(d.day)
								} else if (parseRealDate(d.day.substring(0, d.day.length-4))) {
						...
						this.serverRequest = $.get('http://[fd14:ac28:a278:1:edfb:5a6b:42f4:8436]:8000/api/device-chartt/'+device.id+'/?format=json', function(data) {
							data.forEach(function(d) {
								if (parseDate(d.day)) {
									d.day = parseDate(d.day)
								} else if (parseRealDate(d.day.substring(0, d.day.length-4))) {
			...
			<div className="col-md-12 card dash-panel-height-lg p-0">
				<div className="card-header d-flex flex-row pt-0 chh">
					...
					<div className="pl-5 swidth">
						Date Gradient
						<SliderWithTooltip min={0} max={5} marks={{0: 'None', 1: '5m', 2: '15m', 3: '30m', 4: '1h', 5: '1d'}} onAfterChange={this.handleDateSlider.bind(this)} defaultValue={this.state.date_sl_val} tipProps={{placement: 'bottom', prefixCls: 'rc-slider-tooltip'}}/>
					</div>
					<div className="pl-5 swidth">
						<span>
							<span>
						...
						<SliderWithTooltip min={1} max={500} marks={{1: '1', 500: '500'}} onAfterChange={this.handleBucketSlider.bind(this)} defaultValue={this.state.grade_sl_val} disabled={this.state.dis_b_grad} tipProps={{placement: 'bottom', prefixCls: 'rc-slider-tooltip'}}/>
					</div>
